...
class SoundSystem {
  ...
  playWordAudio(wordId, onEnded = null) {
    ...
    // 處理空白檔名如 "wake up" -> "V5_wake%20up.mp3"
    const fileName = `V5_${wordId}.mp3`;
    const audioPath = `V5_flashcards_audios/${fileName}`;
    ...
    audio.onerror = () => {
      // 嘗試備援父目錄路徑
      const backupPath = `../V5_flashcards_audios/${fileName}`;
  ...
  playZhAudio(wordId, onEnded = null) {
    ...
    const fileName = `V5_${wordId}_zh.mp3`;
    const audioPath = `V5_flashcards_audios/${fileName}`;
    const audio = new Audio(encodeURI(audioPath));
    this.currentVoice = audio;
